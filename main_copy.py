...
## CREATE TABLE
class User(db.Model):
    id = db.Column(BIGINT, primary_key=True, unique=True, autoincrement=True)
...
def callback():
    ...
    if user is None:
        ...
        user = User(id=id_info["sub"], email=id_info['email'], password=password)
        db.session.add(user)
        # db.session.commit()
        
    # Update user object with new data
    # user.google_id = id_info["sub"]
    id_info["sub"] = int(id_info["sub"])
    user.id = id_info["sub"]
